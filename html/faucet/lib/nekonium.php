...
require_once(dirname(__FILE__) . '/json-ipc.php');

class Nekonium extends JSON_IPC
{
	...
	function personal_unlockAccount($address, $password, $duration=null) {
	    if ($duration === null) {
            $param = array($address, $password);
        } else {
            $param = array($address, $password, $duration);
        }

	    return $this->ether_request(__FUNCTION__, $param);
    }
...
class Ethereum_Transaction
{
	...
	function __construct($from, $to, $gas=null, $gasPrice=null, $value=null, $data=null, $nonce=null)
	{
		$this->from = $from;
		$this->to = $to;
		$this->gas = $gas;
	...
	function toArray()
	{
        ...
        if (!is_null($this->gas)) $rtn[0]['gas'] = $this->gas;
	    if (!is_null($this->gasPrice)) $rtn[0]['gasPrice'] = $this->gasPrice;
        if (!is_null($this->value)) $rtn[0]['value'] = $this->value;
        if (!is_null($this->data)) $rtn[0]['data'] = $this->data;
        if (!is_null($this->nonce)) $rtn[0]['nonce'] = $this->nonce;
